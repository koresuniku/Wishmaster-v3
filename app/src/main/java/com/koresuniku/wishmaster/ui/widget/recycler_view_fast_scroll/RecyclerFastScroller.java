package com.koresuniku.wishmaster.ui.widget.recycler_view_fast_scroll;
...
public class RecyclerFastScroller extends FrameLayout {
        ...
        mHandle.setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if (event.getActionMasked() == MotionEvent.ACTION_DOWN) {
                    ...
                    if (Math.abs(dY) >= DEFAULT_FLING_SCROLL_DELTA && mRecyclerView != null) {
                        mLastDY += dY;
                        int averageItemHeight = mRecyclerView.computeVerticalScrollRange() /
                                mRecyclerView.getAdapter().getItemCount();
                        int positionsToScroll = (int) Math.floor(Math.abs(mLastDY) / averageItemHeight);
                        int rest = Math.abs(mLastDY) - (positionsToScroll * averageItemHeight);
                        if (positionsToScroll > 0) {
                            updateRvScrollByPositions(positionsToScroll, mLastDY > 0);
                            mLastDY = rest;
                        }
                    } else {
                        if (mLastDY != 0) updateRvScroll(mLastDY);
                        mLastDY = 0;
    ...
    void updateRvScrollByPositions(int positionsToScroll, boolean upwards) {
        if (mRecyclerView != null && mHandle != null) {
            int currentPosition = ((LinearLayoutManager) mRecyclerView.getLayoutManager()).findFirstCompletelyVisibleItemPosition();
            ((LinearLayoutManager) mRecyclerView.getLayoutManager()).scrollToPositionWithOffset(
                    upwards ? currentPosition + positionsToScroll : currentPosition - positionsToScroll, 0);
        }
    }
}